 ...
 * @method \App\Model\Entity\Registry|bool save(\Cake\Datasource\EntityInterface $entity, $options = [])
...
class RegistryTable extends Table
{
    ...
    public function validationDefault(Validator $validator)
    {
        ...
        $validator
            ->scalar('entry_key')
            ->maxLength('entry_key', 181)
            ->requirePresence('entry_key', 'create')
            ->notEmpty('entry_key')
            ->add('entry_key', 'unique', ['rule' => 'validateUnique', 'provider' => 'table']);

        $validator
            ->scalar('entry_value')
            ->requirePresence('entry_value', 'create')
            ->notEmpty('entry_value');
    ...
    {
        $rules->add($rules->isUnique(['entry_key']));
